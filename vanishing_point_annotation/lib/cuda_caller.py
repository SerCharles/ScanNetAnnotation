...
def get_ceiling_and_floor(layout_seg, lines, ceiling_id, floor_id):
    ...
    line_bottom_x = np.from_array(lines[:, 1], dtype=np.float32)
    line_top_x = np.from_array(lines[:, 0], dtype=np.float32)
    layout_seg = layout_seg.reshape(H * W).astype(np.int32)
    ...
    ceilings_y = np.zeros((2 * W), dtype=np.float32)
    ceilings_x = np.zeros((2 * W), dtype=np.float32)
    floors_y = np.zeros((2 * W), dtype=np.float32)
    floors_x = np.zeros((2 * W), dtype=np.float32)
    ...
    ceiling_places = np.concatenate((ceilings_y, ceilings_x), axis=0)
    floor_places = np.concatenate((floors_y, floors_x), axis=0)
